...
const TodayMealItem = ({item,handleChangeValue, onClickPlus, onClickMinus, onClickCancel}) => {
  return (
      <div className="bg-white rounded-[10px] border border-neutral-200 p-5 flex justify-between w-160">
        <div className="flex gap-5">
          ...
          <div className="flex gap-3">
            <OvalLineButton src={icMinus} size={3} onClick={onClickMinus} />
            <input
                type="number"
            ...
            <OvalLineButton src={icPlus} size={3} onClick={onClickPlus} />
          </div>
        </div>
        <div className="flex gap-2 items-center">
          <p className="text-black text-base font-semibold">{Math.round(item?.kcal)}kcal</p>
          <img src={icClose} className="w-5 h-5 cursor-pointer" onClick={onClickCancel} />
        </div>
      </div>
  );
};
